...
public class GridStatusServlet extends RegistryBasedServlet {
  ...
  public GridStatusServlet(Registry registry) {
    super(registry);
  }
  ...
  private JSONObject getResponse(HttpServletRequest request) throws IOException, JSONException {
    ...
    try {
      if (request.getInputStream() != null) {
        ...
        res.put("sessionsWaiting", numUnprocessedRequests);
         res.put("slots", getAvailableAndTakenSlots());
      }
    } catch (Exception e) {
  ...
  private JSONObject getAvailableAndTakenSlots() throws JSONException {
    int freeSlots = 0;
    int totalSlots = 0;

    JSONObject result = new JSONObject();
    JSONArray availableSlots = new JSONArray();
    JSONArray takenSlots = new JSONArray();

    for (RemoteProxy proxy : getRegistry().getAllProxies()) {
      for (TestSlot slot : proxy.getTestSlots()) {
        JSONObject slotJSON = new JSONObject();
        for (Map.Entry<String, Object> entry : slot.getCapabilities().entrySet()) {
          String key = entry.getKey();
          Object value = entry.getValue();
          slotJSON.put(key, value);
        }
        if (slot.getSession() == null) {
          availableSlots.put(slotJSON);
        } else {
          takenSlots.put(slotJSON);
        }
      }
    }

    result.put("available", availableSlots);
    result.put("taken", takenSlots);
